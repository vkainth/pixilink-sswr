...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `https://${domain}/townhouses-for-sale/${subarea}/${bedsNum}-bedrooms`
  const area = subareaDisplayName(subarea)

  const title = `${bedsNum} Bedroom Townhouses for Sale in ${area} | ${shortArea} | ${agentName}`
  const description = `Browse ${bedsNum}-bedroom townhouses for sale in ${area}, ${shortArea}. Live MLS® listings updated every 5 minutes. Contact ${agentName.split(' ')[0]} to book a private showing.`

  // See condos-for-sale/[subarea]/[beds]/page.tsx for the noindex rationale.
  let noindex = false
  try {
    const { total } = await getListings(slug, { status: 'Active', type: 'Townhouse', subarea: fromSubareaSlug(subarea), beds: bedsNum, limit: 1, noFallback: true })
    noindex = total === 0
  } catch {
    noindex = false
  }
...
export default async function TownhousesSubareaBedsPage({ params, searchParams }: Props) {
  const { slug, subarea, beds } = await params
  const isKnown = SUBAREA_MAP.some(e => e.slug === subarea)
  const bedsNum = parseBeds(beds)
