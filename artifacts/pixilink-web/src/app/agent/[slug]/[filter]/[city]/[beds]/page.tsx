...
interface Props {
  params: Promise<{ slug: string; filter: string; city: string; beds: string }>
...
/** Parse "2-bedroom" → 2, returns null for invalid/unrecognised segments */
function parseBedSegment(segment: string): number | null {
  const m = segment.match(/^(\d+)-bedroom$/)
  if (!m) return null
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const bedsN = parseBedSegment(beds)
  if (!meta || bedsN === null) return { title: 'Page Not Found' }
  const agent = await getAgent(slug)
  const cityDisplay = subareaDisplayName(city)
  const title = `${bedsN}-Bedroom ${cityDisplay} ${meta.plural} for Sale — ${agent?.name || 'Your Agent'}`
  const description = `Browse ${bedsN}-bedroom ${meta.plural.toLowerCase()} for sale in ${cityDisplay}. Real MLS® listings with photos, prices and open house dates from ${agent?.name || 'your local agent'}.`
  const domain = agentCanonicalBase(agent)
  const canonical = `https://${domain}/${filter}/${city}/${beds}`
  return {
    title,
    description,
...
export default async function PropertyTypeCityBedsPage({ params, searchParams }: Props) {
  ...
  const domain = agentCanonicalBase(agent)
  const pageUrl = `https://${domain}/${filter}/${city}/${beds}`
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: `${cityDisplay} ${meta.plural}`, item: `https://${domain}/${filter}/${city}` },
      { '@type': 'ListItem', position: 4, name: `${bedsN}-Bedroom ${cityDisplay} ${meta.plural}`, item: pageUrl },
    ],
  }
  ...
  const jsonLd = {
    ...
    name: `${bedsN}-Bedroom ${cityDisplay} ${meta.plural} for Sale`,
    description: `Active ${bedsN}-bedroom ${meta.plural.toLowerCase()} for sale in ${cityDisplay} — MLS® listings from ${agent.name}.`,
    url: pageUrl,
      ...
      <div style={{ background: '#fff', padding: '48px 0 40px', borderBottom: '1px solid var(--border)' }}>
        <div className="container">
          <div style={{ fontSize: 11, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#888', marginBottom: 12, fontWeight: 500 }}>
            {cityDisplay} · {meta.h1Type} · {bedsN} Bedroom
          </div>
          <h1 className={playfair.className} style={{ fontSize: 'clamp(24px,3.5vw,44px)', fontWeight: 400, lineHeight: 1.15, maxWidth: 680, color: '#1a1a1a', marginBottom: 10 }}>
            {bedsN}-Bedroom {cityDisplay} {meta.plural} for Sale
          </h1>
          <p style={{ color: '#555', fontSize: 14, maxWidth: 600, lineHeight: 1.7, margin: 0 }}>
            Browse all active {bedsN}-bedroom {meta.plural.toLowerCase()} in {cityDisplay}. Updated in real time from the MLS® — photos, floor plans, open house dates and contact {firstName} direct.
          </p>
        </div>
      </div>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid var(--border)', padding: '10px 0', position: 'sticky', top: 0, zIndex: 20 }}>
        <div className="container" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 5 }}>
            {bedOpts.map(b => (
              <a key={b.v} href={bedLink(b.v)} style={chipStyle(b.v === String(bedsN))}>
                {b.l} bd
              </a>
            ))}
          </div>
        ...
        <div>
          ...
          {listings.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px 24px', background: '#fff', borderRadius: 10, border: '1px solid var(--border)' }}>
              ...
              <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--primary-bg)', marginBottom: 8 }}>
                No {bedsN}-Bedroom {meta.plural} Available
              </h2>
              <p style={{ color: 'var(--text-muted)', fontSize: 14, maxWidth: 340, margin: '0 auto 20px' }}>
                There are no active {bedsN}-bedroom {meta.plural.toLowerCase()} in {cityDisplay} right now. Check back soon or contact {firstName} to be notified when new listings hit the market.
              </p>
              <a href={ap(`/${filter}/${city}`)} style={{ background: 'var(--cta-primary)', color: 'var(--cta-primary-text)', padding: '10px 22px', borderRadius: 6, fontWeight: 700, fontSize: 13, textDecoration: 'none' }}>
                All {cityDisplay} {meta.plural}
              </a>
            </div>
          ) : (
            <>
          ...
          <div style={{ ...card, marginTop: 32 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {[
                { l: `All ${cityDisplay} ${meta.plural}`, h: ap(`/${filter}/${city}`) },
