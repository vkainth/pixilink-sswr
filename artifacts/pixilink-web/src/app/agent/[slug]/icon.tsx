...
export default async function AgentIcon({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const initialsEl = (
    ...
    >
      <span style={{ color: brandAccent, fontSize: 18, fontWeight: 800, fontFamily: 'sans-serif' }}>
        {initial}
      </span>
    </div>
  )
  ...
  if (faviconUrl) {
    ...
    if (faviconUrl.startsWith('/')) {
      imgBuf = await readPublicFile(faviconUrl)
