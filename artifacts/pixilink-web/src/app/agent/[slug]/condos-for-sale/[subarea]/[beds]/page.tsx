...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const canonical = `https://${domain}/condos-for-sale/${subarea}/${bedsNum}-bedrooms`
  const area = subareaDisplayName(subarea)

  const title = `${bedsNum} Bedroom Condos for Sale in ${area} | ${shortArea} | ${agentName}`
  const description = `Browse ${bedsNum}-bedroom condos for sale in ${area}, ${shortArea}. Live MLS® listings updated every 5 minutes. Contact ${agentName.split(' ')[0]} to book a private showing.`

  // Zero-inventory combos are thin/duplicate-content pages — keep them live for
  // visitors (alerts/CTA still useful) but ask crawlers not to index them until
  // inventory appears. subarea is always set here, so getListings throws
  // TransientBackendError on a real backend failure rather than silently
  // returning fake fallback data — on that path we deliberately do NOT noindex,
  // since we can't tell if it's really zero.
  let noindex = false
  try {
    const { total } = await getListings(slug, { status: 'Active', type: 'Apartment', subarea: fromSubareaSlug(subarea), beds: bedsNum, limit: 1, noFallback: true })
    noindex = total === 0
  } catch {
    noindex = false
  }
...
export default async function CondosSubareaBedsPage({ params, searchParams }: Props) {
  const { slug, subarea, beds } = await params
  const isKnown = SUBAREA_MAP.some(e => e.slug === subarea)
  const bedsNum = parseBeds(beds)
