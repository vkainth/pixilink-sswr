...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const [agent, enriched, soldStats] = await Promise.all([
    getAgent(slug),
    getLandingPageEnriched(slug, city),
  ...
  const { page } = enriched
  const locationName = page.city_display_name
  const province = page.province || 'BC'
  const siteName = deriveSiteName(agent, locationName)
  ...
  const canonical = agent.settings?.custom_domain
    ? `https://${agent.settings.custom_domain}/top-realtor/${city}`
    : `https://website.pixilink.com/agent/${slug}/top-realtor/${city}`
...
export default async function TopRealtorCityPage({ params }: Props) {
  const { slug, city } = await params
