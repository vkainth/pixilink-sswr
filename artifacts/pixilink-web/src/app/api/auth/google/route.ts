...
export async function GET(req: NextRequest) {
  ...
  try {
    laravelRes = await fetch(
      `${LARAVEL}/api-internal/auth/google/redirect?slug=${encodeURIComponent(slug)}`,
    )
  } catch {
    return NextResponse.redirect(
      new URL(`${BASE}/agent/${slug}/sign-in?google_error=service_unavailable`, req.url),
    )
  }
  ...
  if (!laravelRes.ok || !data.url) {
    return NextResponse.redirect(
      new URL(`${BASE}/agent/${slug}/sign-in?google_error=not_configured`, req.url),
    )
  }

  // Store slug + return_to so the complete route can issue correct error redirects
  // and restore the user's intended destination, even after the cross-domain round-trip
  // through website.pixilink.com (the central OAuth callback domain).
  const cookiePayload = JSON.stringify({ slug, returnTo })

  const response = NextResponse.redirect(data.url)
  response.cookies.set('pxl_oauth_ctx', cookiePayload, {
    maxAge: 600,
    httpOnly: true,
    sameSite: 'lax',
    path: '/',
